        ...
        try:
            ...
            if images and any(Path(img).exists() for img in images):
                ...
                if system_instructions:
                    content_parts.append({"text": system_instructions})
                ...
                for img_path in valid_images:
                    try:
                        with open(img_path, "rb") as f:
                            image_bytes = f.read()
                        content_parts.append({"image": {"mime_type": "image/jpeg", "data": image_bytes}})
                    except Exception as e:
                        logger.error(f"Failed to read image {img_path}: {str(e)}")
...
class RemedyChatbot:
    ...
    def process_query(self, query):
        ...
        # For other queries, use the LLM to determine what information to retrieve
        ...
        Based on this query, determine what type of information to retrieve from Remedy:
        
        1. Is the user looking for a specific incident by ID? If yes, extract the incident ID.
        2. Is the user looking for incidents from a specific date? If yes, extract the date.
        3. Is the user looking for incidents with a particular status? If yes, extract the status.
        4. Is the user looking for incidents assigned to a specific person? If yes, extract the person's name.
        5. Is the user searching for incidents related to a specific topic or containing specific text? If yes, extract the search terms.
        6. Is the user looking for incidents assigned to a specific support group? If yes, extract the group name.
        7. Is the user looking for recent incidents? If yes, determine the time frame.
        8. Is the user asking about incident history or attachments? If yes, extract the relevant incident ID.
        
        Return your analysis in JSON format with the following structure:
        {{
            "query_type": "one of [incident_id, date, status, assignee, search, support_group, recent, history, unknown]",
            "parameters": {{...}} # relevant parameters for the query type
        }}
        
        Only respond with valid JSON. Do not include any other text.
        """
        
        # Get analysis of the query
        analysis_response = self.gemini_ai.generate_response(retrieval_prompt)
        ...
        try:
            ...
            logger.error(f"Failed to parse query analysis: {analysis_response}")
            analysis = {"query_type": "unknown", "parameters": {}}
            
        # Retrieve the requested information based on the query type
        retrieved_data = self._retrieve_data(analysis)
        ...
        if retrieved_data:
            ...
            I was unable to retrieve the requested information from Remedy. Please provide a helpful response explaining why this might be the case and suggesting alternatives or asking for clarification. Be friendly and helpful.
            """
